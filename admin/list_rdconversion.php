...
$.ajax({
		type:"GET",
		url:"userInfo.php?action=close_process",
 		data:"row_id="+row_id,
...
<div class="top-bar">
  ...
  <a href="topic_genrate.php" class="button">ADD Topic</a>
  <h1>R & D Conversation</h1>
</div>
<div class="top-bar">
  <div style="float:right";><font style="color:#FFFF9D;font-weight:bold;">Yellow:</font> Back from R & D</div>
  <br/>
...
if($_POST["Showrequest"]==1)
 ...
 {
	 $WhereQuery=" where status=1 and close_status!=1";
 }
 else
 { 
	  
   $WhereQuery=" where status in ('1','2') and close_status=0";
...
{
?>
      <tr align="center" <? if($query[$i]["close_status"]==1){ echo 'style="background-color:#99FF66"';} elseif( $query[$i]["rd_comment"]!="" && $query[$i]["status"]==2 ){ echo 'style="background-color:#FFFF9D"';} ?> >
        <td><?php echo $i+1; ?></td>
        ...
        <td><!--<a href="#" onclick="Show_record(<?php echo $query[$i]["id"];?>,'rd_conversion','popup1'); " class="topopup">View Detail</a>-->
          ...
          <a href="#" onclick="return ConfirmDelete(<?php echo $query[$i]["id"];?>);"  >Close Process</a>
          <?  if( $query[$i]["rd_comment"]!="" && $query[$i]["status"]=="2"){ ?>
