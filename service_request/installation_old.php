...
//echo "SELECT * FROM installation_request where (installation_status ='1' or installation_status='2' or installation_status='4' or installation_status='7' or installation_status='8' or installation_status='9') and branch_id=1 and request_by='".$_SESSION['username']."' and req_date < '".$PendingDateService."' order by id desc";die;
//$PendingSql =  select_query("SELECT *,DATE_FORMAT(req_date,'%d %b %Y %h:%i %p') as req_date,DATE_FORMAT(time,'%d %b %Y %h:%i %p') as time FROM installation_request where (installation_status ='1' or installation_status='2' or installation_status='4' or installation_status='7' or installation_status='8' or installation_status='9') and branch_id=1 and request_by='".$_SESSION['username']."' and req_date < '".$PendingDateService."' order by id desc");
//echo "<pre>";print_r($PendingSql);die;
$mode=$_GET['mode'];
if($mode=='') { $mode="new"; }
...
?>
  ...
  ?>
        ...
        <td width="1%" align="center">&nbsp;<?php echo $query[$i]['installation_approve'];?></td>
        ...
        <td width="1%" align="center" nowrap>
          ...
          <?php 
            ...
            // if($query[$i]["model"] == '')
            // {
            //   //echo "select device_status from installation_request WHERE id='".$query[$i]['id']."'";
            //   $sqlDevice=select_query("select device_status from installation_request WHERE id='".$query[$i]['id']."'");
            //   echo $sqlDevice[0]['device_status'];
            // }
            // else
            // {
            ...
            // }
           $sqlDevice=select_query("SELECT item_name FROM item_master where item_id='".$query[$i]["device_type"]."'");
      ...
      <!--   <td width="1%" align="center">
        ...
        <td width="1%" align="center">
          ...
          $sql5 = select_query("select accessories_tollkit from installation_request WHERE id='".$query[$i]['id']."'");
