...
if(isset($_POST["submit"]) && $_POST["submit"]=="submit")
{
   ...
   else
   {
	...
	$get_sales_details = select_query("SELECT * FROM `internalsoftware`.`sales_person` WHERE active='1' and name='".$sales_manager_edit."'");
	
    $branch_id = $get_sales_details[0]['branch_id'];
	...
	if($action=='edit')
	{
		$query="update new_account_creation set company='".$company."', potential='".$potential."', client_gst_no='".$client_gst_no."', 
				...
				accessories_tollkit='".$accessories_tollkits."', sales_manager='".$sales_manager."', branch_id='".$branch_id."' where id=$id";
	...
	{
		$query="update new_account_creation set sales_manager='".$sales_manager."', company='".$company."', potential='".$potential."', 
				type_of_org='".$type_of_org."', pan_no='".$pan_no."', contact_person='".$contact_person."', contact_number='".$contact_number."', 
				billing_name='".$billing_name."', billing_address='".$billing_address."', dimts='".$dimts."', dimts_fee='".$dimts_fee."', 
				vehicle_type='".$vehicle_type."', immobilizer='".$immobilizer."', ac_on_off='".$ac_on_off."',  email_id='".$email_id."', 
				user_name='".$user_name."', user_password='".$user_password."', collection_manager='".$collection_manager."', 
				vehicle_warranty='".$vehicle_warranty."', client_state='".$client_state."', acess_selection='".$acess_selection."', 
				accessories_tollkit='".$accessories_tollkits."', client_gst_no='".$client_gst_no."', branch_id='".$branch_id."'  where id=$id";
	...
	else
	{
		...
		$insert_accountCreation = array('date' => $date, 'account_manager' => $account_manager, 'sales_manager' => $sales_manager, 
			...
			'user_password' => $user_password, 'branch_id' =>  $branch_id, 'vehicle_warranty' => $vehicle_warranty, 
...
<SCRIPT language="javascript">
  ...
  <form action="NewAccountCreation.php" method="post" enctype="multipart/form-data" name="myForm" onsubmit="return Check()" autocomplete="off">
    <table style=" padding-left: 100px;width: 500px;" cellspacing="5" cellpadding="5">
      ...
      <tr>
        <td>Sales Manager:<font color="red">* </font></td>
        <td><select name="sales_manager" id="sales_manager" style="width:150px">
            <option value="" >-- select one --</option>
            <?php
                    $sales_manager = select_query("SELECT name FROM internalsoftware.sales_person where active=1 order by name ");
					for($s=0;$s<count($sales_manager);$s++)
                    {
                    ?>
            <option value="<?=$sales_manager[$s]['name']?>"<? if($result[0]['sales_manager']==$sales_manager[$s]['name']){?> selected="selected" <? }?> > <?php echo $sales_manager[$s]['name']; ?> </option>
            <?php
                    }
                    ?>
          </select></td>
      ...
      <tr>
        ...
        <td><select name="collection_manager" id="collection_manager" style="width:150px">
            <option value="" >-- select one --</option>
            <?php
				$collection_manager = select_query("SELECT id,name FROM internalsoftware.collection_agent where is_active=1 order by name ");
