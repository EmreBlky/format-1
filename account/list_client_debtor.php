...
<div class="top-bar">
  ...
  <a href="pending_debtor.php" class="button" target="_blank">ADD Pending </a> 
...
$query = select_query("select dpb.id,dpb.client_id,dpb.company_name, sp.name as sales_manager,
					 ...
					 dpb.rent_amount_pending, dpb.accessory_amount_pending, dpb.req_time, ad.UserName, ad.Branch_id
...
<?php 
    ...
    $excel_data.='<table cellpadding="5" cellspacing="5" border="1">
					<thead>
						<tr>
							<td colspan="30"></td>
						</tr>
						<tr>
							<td>'.date('d/m/Y').'</td>
							<td>&nbsp;</td>
							<td colspan="12" style="background-color:#09F"><strong>DEVICE PENDING</strong></td>
							<td colspan="12" style="background-color:#0C9"><strong>RENT PENDING</strong></td>
						...
						<tr>
						  ...
						  for($dv=11;$dv>=0;$dv--)
						  {		
						  	$excel_data.='<th style="background-color:#09F">'.date("F y", strtotime(date("Y M", strtotime($startdate))." -".$dv." month")).'</th>';
						  }
						  
						  for($rt=11;$rt>=0;$rt--)
						  {		
						  	$excel_data.='<th style="background-color:#0C9">'.date("F y", strtotime(date("Y M", strtotime($startdate))." -".$rt." month")).'</th>';
						  }
			  
			  $excel_data.='<th style="background-color:#FC3">Yearly</th>
							<th style="background-color:#FC3">Total</th>
...
{	
	...
	$pndg_sub_query = select_query("select dpb.client_id,dpb.company_name, dpb.month,dpb.year, dpb.device_amount_pending, 
						...
						where dpb.client_id='".$query[$i]["client_id"]."'");
	...
	$excel_data.="<tr>
					<td>".$query[$i]["company_name"]."</td>
					<td>".$final_accessory_amount."</td>";
					
	for($dvr=11;$dvr>=0;$dvr--)
	{		
		$month_date = date("Y-m", strtotime(date("Y-m", strtotime($startdate))." -".$dvr." month"));
		$new_value = explode("-",$month_date);
		
		$data_sub_query = select_query("select dpb.client_id,dpb.company_name, dpb.month,dpb.year, dpb.device_amount_pending, 
						dpb.rent_amount_pending from debtor_pending_billing as dpb 
						where dpb.client_id='".$query[$i]["client_id"]."' and dpb.month='".$new_value[1]."' and dpb.year='".$new_value[0]."'");
						
		$excel_data.='<td>'.$data_sub_query[0]["device_amount_pending"].'</td>';
	}
	
	for($rtr=11;$rtr>=0;$rtr--)
	{		
		$rmonth_date = date("Y-m", strtotime(date("Y-m", strtotime($startdate))." -".$rtr." month"));
		$rnew_value = explode("-",$rmonth_date);
		
		$rdata_sub_query = select_query("select dpb.client_id,dpb.company_name, dpb.month,dpb.year, dpb.device_amount_pending, 
						dpb.rent_amount_pending from debtor_pending_billing as dpb 
						where dpb.client_id='".$query[$i]["client_id"]."' and dpb.month='".$rnew_value[1]."' and dpb.year='".$rnew_value[0]."'");
						
		$excel_data.='<td>'.$rdata_sub_query[0]["rent_amount_pending"].'</td>';
	}				
	...
	$excel_data.="<td>".$yearly_rent_amount."</td><td>".$total_amount."</td>
		<td>".$query[$i]["collection_agent"]."</td><td>".$query[$i]["sales_manager"]."</td></tr>";					
...
?>
    ...
    <tr align="center">
        ...
        <td><?php echo $final_rent_amount + $yearly_rent_amount;?></td>
        ...
        <td>
           ...
           <? if($total_amount > 0 ) { ?> 
            | <a href="received_debtor.php?company=<?=$query[$i]["company_name"];?>&manager=<?=$query[$i]["manager"];?>&agent=<?=$query[$i]["agent"];?>&client=<?=$query[$i]["client_id"];?>&AddRecd=true" target="_blank">Received</a> 
            | <a href="discounting_debtor.php?company=<?=$query[$i]["company_name"];?>&manager=<?=$query[$i]["manager"];?>&agent=<?=$query[$i]["agent"];?>&client=<?=$query[$i]["client_id"];?>&AddRecd=true" target="_blank">Discounting</a> 
            <? } ?>
            </td>
      </tr>
      <?php 
...
<?php 
unlink(__DOCUMENT_ROOT.'/account/Billing_data/pending_debtor_list.xls');
$filepointer=fopen(__DOCUMENT_ROOT.'/account/Billing_data/pending_debtor_list.xls','w');
