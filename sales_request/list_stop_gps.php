...
?>
 ...
 <? $sql="SELECT Userid AS id,UserName AS sys_username FROM addclient  WHERE Userid=".$query[$i]["client"];
  ...
  <td><?php echo $query[$i]["reason"];?></td>
  <td><strong>
<?  if($query[0]["stop_gps_status"]==2 || (($query[0]["support_comment"]!="" || $query[0]["admin_comment"]!="") && $query[0]["sales_comment"]==""))
    ...
    elseif($query[0]["approve_status"]==0 && $query[0]["forward_req_user"]!="" && $query[0]["forward_back_comment"]=="" && $query[0]["stop_gps_status"]==1)   
    {echo "Pending Admin Approval (Req Forward to ".$query[0]["forward_req_user"].")";}
    elseif($query[0]["approve_status"]==0 && ($query[0]["account_comment"]!="" || $query[0]["total_pending"]!="") && $query[0]["final_status"]==0 && $query[0]["stop_gps_status"]==1)
    {echo "Pending Admin Approval";}
    elseif($query[0]["approve_status"]==1 && $query[0]["stop_gps_status"]==1 && $query[0]["final_status"]!=1){echo "Pending at Tech Support Team";}
