...
$query = select_query("select dpb.id,dpb.client_id,dpb.company_name, sp.name as sales_manager,
					 ...
					 dpb.rent_amount_pending, dpb.accessory_amount_pending, dpb.req_time, ad.UserName
					 ...
					 on dpb.collection_agent=ca.id left join addclient as ad 
					 on dpb.client_id=ad.Userid where dpb.is_status='1' and dpb.is_active='1' 
					 group by dpb.client_id order by dpb.id");
...
for($i=0;$i<count($query);$i++)
{
		...
		$pndg_sub_query = select_query("select dpb.client_id,dpb.company_name, dpb.month,dpb.year, dpb.device_amount_pending, dpb.rent_amount_pending, dpb.accessory_amount_pending, dpb.yearly_rent, dpb.req_time from debtor_pending_billing as dpb where dpb.client_id='".$query[$i]["client_id"]."'");
			
	$device_amount_pending = 0;
	$rent_amount_pending = 0;
	$accessory_amount_pending = 0;
	$yearly_rent_pending = 0;
	...
	for($sq=0;$sq<count($pndg_sub_query);$sq++)
	{
		...
		$yearly_rent_pending = $yearly_rent_pending + $pndg_sub_query[$sq]["yearly_rent"];
	}
	...
	if($yearly_rent_pending < 0){ $yearly_rent_amount = 0; }else{ $yearly_rent_amount = $yearly_rent_pending; }
	
	$discounting_amount = 0;
	$tds_amount = 0;
	
	$total_amount = ($final_device_amount + $final_rent_amount + $final_accessory_amount + $yearly_rent_amount) - ($discounting_amount + $tds_amount);
		...
		$arr=array (
				...
				'client_name' => $query[$i]["UserName"],
				'company_name' => $query[$i]["company_name"],
				'final_device_amount' => $final_device_amount,
				'final_rent_amount' => $final_rent_amount + $yearly_rent_amount,
	...
	if($mode=="Received")
	{
		...
		if($sales_manager != " " && $collection_agent == " " && $client_name == " ")
		{
			$query_recd = select_query("SELECT drb.id,drb.client_id,drb.company_name, sp.name AS sales_manager,
						...
						drb.rent_amount_recd, drb.accessory_amount_recd, drb.received_time, ad.UserName
						...
						ON drb.collection_agent=ca.id left join addclient as ad 
					 	ON dpb.client_id=ad.Userid WHERE drb.sales_manager='".$sales_manager."' GROUP BY drb.client_id");
		...
		else if($sales_manager == " " && $collection_agent != " " && $client_name == " ")
		{
			$query_recd = select_query("SELECT drb.id,drb.client_id,drb.company_name, sp.name AS sales_manager,
						...
						ON drb.sales_manager=sp.id LEFT JOIN collection_agent AS ca
						ON drb.collection_agent=ca.id left join addclient as ad 
					 	ON dpb.client_id=ad.Userid WHERE drb.collection_agent='".$collection_agent."' GROUP BY drb.client_id");
		...
		else if($sales_manager == " " && $collection_agent == " " && $client_name != " ")
		{
			$query_recd = select_query("SELECT drb.id,drb.client_id,drb.company_name, sp.name AS sales_manager,
						...
						ON drb.sales_manager=sp.id LEFT JOIN collection_agent AS ca
						ON drb.collection_agent=ca.id left join addclient as ad 
					 	ON dpb.client_id=ad.Userid WHERE drb.client_id='".$client_name."' GROUP BY drb.client_id");
		...
		else if($sales_manager != " " && $collection_agent != " " && $client_name == " ")
		{
			$query_recd = select_query("SELECT drb.id,drb.client_id,drb.company_name, sp.name AS sales_manager,
						...
						left join addclient as ad ON dpb.client_id=ad.Userid WHERE drb.sales_manager='".$sales_manager."' AND 
						drb.collection_agent='".$collection_agent."' GROUP BY drb.client_id");
		...
		else if($sales_manager == " " && $collection_agent != " " && $client_name != " ")
		{
			$query_recd = select_query("SELECT drb.id,drb.client_id,drb.company_name, sp.name AS sales_manager,
						...
						FROM debtor_received_billing AS drb LEFT JOIN sales_person AS sp
						ON drb.sales_manager=sp.id LEFT JOIN collection_agent AS ca ON drb.collection_agent=ca.id 
						left join addclient as ad ON dpb.client_id=ad.Userid WHERE drb.client_id='".$client_name."' AND 
						drb.collection_agent='".$collection_agent."' GROUP BY drb.client_id");
		...
		else if($sales_manager != " " && $collection_agent != " " && $client_name != " ")
		{
			$query_recd = select_query("SELECT drb.id,drb.client_id,drb.company_name, sp.name AS sales_manager,
						...
						ON drb.sales_manager=sp.id LEFT JOIN collection_agent AS ca ON drb.collection_agent=ca.id 
						left join addclient as ad ON dpb.client_id=ad.Userid WHERE drb.sales_manager='".$sales_manager."' AND 
						drb.collection_agent='".$collection_agent."' and drb.client_id='".$client_name."' GROUP BY drb.client_id");
		...
		for($qr=0;$qr<count($query_recd);$qr++)
		{
			...
			accessory_amount_recd, yearly_rent, discounting, tds_amount, received_time from debtor_received_billing 
			...
			for($rs=0;$rs<count($recd_sub_query);$rs++)
			{
				...
				$yearly_rent_recd = $yearly_rent_recd + $recd_sub_query[$rs]["yearly_rent"];
			...
			$total_amount = ($device_amount_recd + $rent_amount_recd + $accessory_amount_recd + $yearly_rent_recd) - ($discounting_amount + $tds_amount);
			...
			$arr=array (
					...
					'client_name' => $query_recd[$qr]["UserName"],
					'company_name' => $query_recd[$qr]["company_name"],
					'final_device_amount' => $device_amount_recd,
					'final_rent_amount' => $rent_amount_recd + $yearly_rent_recd,
	...
	else
	{
		...
		if($sales_manager != " " && $collection_agent == " " && $client_name == " ")
		{
			$query_pending = select_query("select dpb.id,dpb.client_id,dpb.company_name, sp.name as sales_manager,
						 ...
						 on dpb.sales_manager=sp.id left join collection_agent as ca
						 on dpb.collection_agent=ca.id left join addclient as ad 
					 	 on dpb.client_id=ad.Userid WHERE dpb.sales_manager='".$sales_manager."' GROUP BY dpb.client_id");
		...
		else if($sales_manager == " " && $collection_agent != " " && $client_name == " ")
		{
			$query_pending = select_query("select dpb.id,dpb.client_id,dpb.company_name, sp.name as sales_manager,
						 ...
						 on dpb.sales_manager=sp.id left join collection_agent as ca
						 on dpb.collection_agent=ca.id left join addclient as ad 
					 	 on dpb.client_id=ad.Userid WHERE dpb.collection_agent='".$collection_agent."' GROUP BY dpb.client_id");
		...
		else if($sales_manager == " " && $collection_agent == " " && $client_name != " ")
		{
			$query_pending = select_query("select dpb.id,dpb.client_id,dpb.company_name, sp.name as sales_manager,
						 ...
						 on dpb.sales_manager=sp.id left join collection_agent as ca
						 on dpb.collection_agent=ca.id left join addclient as ad 
					 	 on dpb.client_id=ad.Userid WHERE dpb.client_id='".$client_name."' GROUP BY dpb.client_id");
		...
		else if($sales_manager != " " && $collection_agent != " " && $client_name == " ")
		{
			$query_pending = select_query("select dpb.id,dpb.client_id,dpb.company_name, sp.name as sales_manager,
						 ...
						 left join addclient as ad on dpb.client_id=ad.Userid WHERE dpb.sales_manager='".$sales_manager."' AND 
						 dpb.collection_agent='".$collection_agent."' GROUP BY dpb.client_id");
		...
		else if($sales_manager != " " && $collection_agent == " " && $client_name != " ")
		{
			$query_pending = select_query("select dpb.id,dpb.client_id,dpb.company_name, sp.name as sales_manager,
						 ...
						 on dpb.sales_manager=sp.id left join collection_agent as ca on dpb.collection_agent=ca.id 
						 left join addclient as ad on dpb.client_id=ad.Userid WHERE dpb.sales_manager='".$sales_manager."' AND 
						 dpb.client_id='".$client_name."' GROUP BY dpb.client_id");
		...
		else if($sales_manager == " " && $collection_agent != " " && $client_name != " ")
		{
			$query_pending = select_query("select dpb.id,dpb.client_id,dpb.company_name, sp.name as sales_manager,
						 ...
						 from debtor_pending_billing as dpb left join sales_person as sp
						 on dpb.sales_manager=sp.id left join collection_agent as ca on dpb.collection_agent=ca.id 
						 left join addclient as ad on dpb.client_id=ad.Userid WHERE dpb.client_id='".$client_name."' AND 
						 dpb.collection_agent='".$collection_agent."' GROUP BY dpb.client_id");
		...
		else if($sales_manager != " " && $collection_agent != " " && $client_name != " ")
		{
			$query_pending = select_query("select dpb.id,dpb.client_id,dpb.company_name, sp.name as sales_manager,
						 ...
						 on dpb.sales_manager=sp.id left join collection_agent as ca on dpb.collection_agent=ca.id 
						 left join addclient as ad on dpb.client_id=ad.Userid WHERE dpb.sales_manager='".$sales_manager."' AND 
						 dpb.collection_agent='".$collection_agent."' and dpb.client_id='".$client_name."' GROUP BY dpb.client_id");
		...
		for($qp=0;$qp<count($query_pending);$qp++)
		{
				...
				dpb.rent_amount_pending, dpb.accessory_amount_pending, dpb.yearly_rent, dpb.req_time from debtor_pending_billing as dpb 
			...
			$arr=array (
					...
					'client_name' => $query_pending[$qp]["UserName"],
...
?>
      <tr align="center">
        ...
        <td><?php echo $rslt_data[$fn]["client_name"];?></td>
