...
if(isset($_GET['action']) && $_GET['action']=='toolsAccessories')
{
  ...
  $sql=select_query("select access_toolkit from new_account_creation where user_id=".$q);

  $toolkitId = explode("#",$sql[0]['access_toolkit']);
...
{ 
  ...
  $sql2="SELECT distinct dtype.item_id as dev_type_id,dtype.item_name as deviceType FROM new_account_model_master as newmodel LEFT JOIN item_master as dtype ON newmodel.device_type=dtype.item_id WHERE new_account_reqid='".$userId."'"; 
    //echo $sql2; die;
    $row2=select_query($sql2);  
    echo json_encode($row2);
}
...
{ 
    ...
    // echo json_encode($row2);
      ...
      $sql2="SELECT dm.item_id as model_id,dm.item_name as model_name from new_account_model_master as newmodel inner join item_master as dm  ON newmodel.device_model=dm.item_id WHERE newmodel.new_account_reqid='".$userId1."' and dm.parent_id='".$dev_type_id."'" ;
    //echo $sql2; die;
    $row2=select_query($sql2);  
    echo json_encode($row2);
}
...
if(isset($_GET['action']) && $_GET['action']=='InstallationConfirm')
{
  //echo 'tt'; die;
    ...
    //         $installation = "INSERT INTO internalsoftware.installation(`inst_req_id`, `req_date`, `request_by`, sales_person, `user_id`,`company_name`, no_of_vehicals, location, model,time,contact_number,installed_date,status,contact_person,veh_type,required,branch_id,installation_status, Zone_area,atime_status,`inter_branch`,branch_type, instal_reinstall, designation, device_type, alter_contact_no, accessories_tollkit, billing, TrailerType, TruckType, MachineType, actype, standard, alt_cont_person, alt_designation, acess_selection, installation_approve) VALUES('".$row[0]["id"]."','".$row[0]["req_date"]."','".$row[0]["request_by"]."','".$row[0]["sales_person"]."','".$row[0]["user_id"]."', '".$row[0]["company_name"]."','1','".$row[0]["location"]."','".$row[0]["model"]."','".$row[0]["time"]."','".$row[0]["contact_number"]."','".$row[0]["installed_date"]."', 1,'".$row[0]["contact_person"]."','".$row[0]["veh_type"]."','".$row[0]["required"]."','".$row[0]["branch_id"]."','1','".$row[0]["Zone_area"]."','".$row[0]["atime_status"]."','".$row[0]["inter_branch"]."','".$row[0]["branch_type"]."','".$row[0]["instal_reinstall"]."','".$row[0]['designation']."','".$row[0]['device_type']."','".$row[0]['alter_contact_no']."','".$row[0]['accessories_tollkit']."','".$row[0]['billing']."','".$row[0]['TrailerType']."','".$row[0]['TruckType']."','".$row[0]['MachineType']."','".$row[0]['actype']."','".$row[0]['standard']."','".$row[0]['alt_cont_person']."','".$row[0]['alt_designation']."','".$row[0]['acess_selection']."',1)";
    //           // echo $installation; die;
...
if(isset($_GET['action']) && $_GET['action']=='getAllImei')
  {    
    ...
    for ($i=0; $i < $rowCount; $i++) { 
      array_push($device_imei_data, $row[$i]);
    }

    for($j=0;$j<count($device_imei_data);$j++)
        {
          $device.="'".$device_imei_data[$j]['imei']."',";
        }
         $arrDeviceStr=substr($device,0,strlen($device)-1);
        //echo $arrDeviceStr;
    
    if($deviceStatus == 1){
      $result=select_inventory_query("SELECT device_imei FROM inventory.device WHERE device_status IN (57,63,64) and device_imei IN ($arrDeviceStr)");
    }
    else{
      $result=select_inventory_query("SELECT device_imei FROM inventory.device WHERE device_status IN (103) and device_imei IN ($arrDeviceStr)");
    }  
...
{    
    $sql="select imei from internalsoftware.deletion where user_id=".$row_id;  
    $row=select_query($sql);  
    echo json_encode($row);
}
...
<div id="databox">
 ...
 -->
        ...
        else if($row[0]["instal_reinstall"] == 're_install'){
