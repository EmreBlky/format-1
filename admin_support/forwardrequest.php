...
elseif($req_id=='14')
 {
		...
		{
			$query14="update discount_details set forward_req_user='".$requested_user."',forward_comment='".$forward_comment."' where id=$id";
			mysql_query($query14);
			echo "Request forwarded successfully";
		}
		else{
			
			$inser_query14 = mysql_query("INSERT INTO forward_req_history(table_name, table_row_id, forward_req_user, forward_comment, forward_back_comment)
            VALUES('discount_details','".$id."','".$row14[0]["forward_req_user"]."','".$row14[0]["forward_comment"]."','".$row14[0]["forward_back_comment"]."')");

			$query14="update discount_details set forward_req_user='".$requested_user."',forward_comment='".$forward_comment."',forward_back_comment='' where id=$id";
			 mysql_query($query14);
			echo "Request forwarded successfully";
		}
...
<tr>
 <td>
  <select name="requested_user" id="requested_user" width="150px">
        ...
        <?php
        $main_city = select_query("select login_name from requested_users where active_status=1 order by login_name");
        //while($data=mysql_fetch_assoc($main_city))
		for($i=0;$i<count($main_city);$i++)
        {
			  ?>
        
        <option value ="<?php echo $main_city[$i]['login_name'] ?>"  <?echo $selected;?>>
        <?php echo $main_city[$i]['login_name']; ?>
        </option>
        <?php 
        } 
