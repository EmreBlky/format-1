...
if(isset($_REQUEST["id"]))
{
  ...
  if($branch_id==0){
    ...
    // $modelName = $sqlModel[0]['device_model'];
     $sqlModel=select_query("SELECT item_name FROM item_master where item_id='".$deviceModel."'");
     $modelName = $sqlModel[0]['item_name'];



    $query = "SELECT * FROM admin_stock_view where dispatch_branch='".$branch_id."'";
    //echo $query; die;
  ...
  <table class="table table-bordered table-striped">
    ...
    <tbody id="myTable">
      <?php for($i=0;$i<count($row);$i++) { 
        ...
        ?>  
      <tr <?php if($modelName == $row[$i]['device_type']){ echo "style='background-color:#f4aa42'"; }else{ echo "style=''"; } ?> >
