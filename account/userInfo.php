...
if(isset($_GET['action']) && $_GET['action']=='pending_debtor')
{
	...
	$pending_amount_data = select_query("select * from debtor_pending_billing where client_id='".$q."' and (device_amount_pending > 0 or rent_amount_pending > 0 or accessory_amount_pending > 0 or yearly_rent > 0) order by year,month desc");
...
if(isset($_GET['action']) && $_GET['action']=='deactivateUserAccount')
{
	...
	//$deactivate_user = select_query("update matrix.group_users set active=0 where group_users.sys_user_id='".$row_id."'",$dblink);
    ...
    select_query($Updateapprovestatus,$dblink);*/
 ...
 if(isset($_GET['action']) && $_GET['action']=='VehNoChangePaymentClear')
{
    ...
    $Updateapprovestatus="update vehicle_no_change  set account_comment='".$row[0]["account_comment"]."<br/>".date("Y-m-d H:i:s")." - " .$comment."' where id=".$row_id;*/
     ...
     if(select_query($Updateapprovestatus,$dblink2))
     echo "Comment Added Successfully";
    
 }
 
if(isset($_GET['action']) && $_GET['action']=='DebtorAmountRecd')
{
     $Updateapprovestatus = "UPDATE `internalsoftware`.`collection_received_billing` SET `is_status`='0', `is_active`='0' WHERE `id`=".$row_id;
     select_query($Updateapprovestatus);
     echo "Req Closed Successfully";
...
<tr><td>Rent Pending</td><td><? echo (int)$pndg_sub_query[$sq]["rent_amount_pending"] + $pndg_sub_query[$sq]["yearly_rent"];?></td></tr>     
...
<tr><td>Rent Amount Recd</td><td><? echo (int)$recd_sub_query[$rs]["rent_amount_recd"] + $recd_sub_query[$rs]["yearly_rent"];?></td></tr>     
