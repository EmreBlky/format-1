...
mysql_query("Update billing set final_status ='1' where id ='".$chk_id[$i]."'");
}
}
...
$num=mysql_num_rows(mysql_query("SELECT * FROM billing where final_status!=1 and approve_status=1"));
...
$query = mysql_query("SELECT * FROM billing where final_status!=1 and approve_status=1 order by id desc LIMIT $start,$end");
...
<td><?php echo $row["date"];?></td>
  ...
  <td><?php echo $row["retail_maintenance"];?></td>
  <td><?php echo $row["spare_parts"];?></td>
  <td><?php echo $row["spare_costs"];?></td>
