...
if($action=='edit')
{
        ...
        $InstallerId = $result[0]['inst_id'];
                        
        if($_SESSION['user_name']=="triloki"){
            $BranchId = "1";
        } else if($_SESSION['user_name']=="pankaj"){
            $BranchId = "2";
        } else if($_SESSION['user_name']=="jaipurrequest"){
            $BranchId = "3";
        } else if($_SESSION['user_name']=="asaleslogin"){
            $BranchId = "6";
        } else if($_SESSION['user_name']=="ksaleslogin"){
            $BranchId = "7";
        }
    ...
    device.sim_id=sim.sim_id where  device.device_status=64 and ChallanDetail.branchid=".$BranchId." and  
    ChallanDetail.CurrentRecord=1 and ChallanDetail.installerid=".$InstallerId);
        
        //echo "<pre>";print_r($assign_device);die;
...
if(isset($_POST["submit"]))
{
        ...
        if($Device_type=="New")
         {
               ...
               $get_device_id = select_query_inventory("select device.device_id, device.device_imei, device.itgc_id, device.dispatch_date from inventory.device where device.device_imei='".trim($rdd_device_imei)."' ");    
                           
               $query="update device_change set sales_manager='".$sales_manager."', device_model='".$device_model."', rdd_device_model='".$rdd_device_model."', rdd_device_id='".$get_device_id[0]['itgc_id']."', rdd_device_imei='".$rdd_device_imei."', rdd_device_type='".$Device_type."',rdd_date='".$Txtrdd_date."', rdd_reason='".$TxtReason."', billing='".$billing."', req_fill_date='".$date."' where id=$id";
              
               select_query($query);           
           
               $new_imei_query_inv = select_query_inventory("update inventory.device set device_status=65 where device_imei='".trim($rdd_device_imei)."' and device_status not in(84,85,86,79,68,105,62,70,94,95,83,81,66,75,82,57,63,116,103,194,195,196,19)");
      
               $new_imei_repair_inv = select_query_inventory("update inventory.device_repair set device_status=65 where current_record=1 and device_imei='".trim($rdd_device_imei)."' and device_status not in(84,85,86,79,68,105,62,70,94,95,83,81,66,75,82,57,63,116,103,194,195,196,19)");
         ...
         elseif($Device_type=="FFC")
         {
                ...
                $query="update device_change set sales_manager='".$sales_manager."', device_model='".$device_model."', rdd_device_model='".$rdd_ffc_device_model."', rdd_device_imei='".$rdd_ffc_device_imei."', last_ffc_user='".$last_ffc_user."', ffc_veh_no='".$ffc_veh."', ffc_date='".$ffc_date."', rdd_device_type='".$Device_type."',rdd_date='".$Txtrdd_date."', rdd_reason='".$TxtReason."',billing='".$billing_ffc."', billing_reason='".$billing_reason."', req_fill_date='".$date."' where id=$id";
              
                select_query($query);
                              
                $new_imei_query_inv = select_query_inventory("update inventory.device set device_status=65 where device_imei='".trim($rdd_ffc_device_imei)."' and device_status not in(84,85,86,79,68,105,62,70,94,95,83,81,66,75,82,57,63,116,103,194,195,196,19)");
      
                $new_imei_query_inv2 = select_query_inventory("update inventory.device_repair set device_status=65 where current_record=1 and device_imei='".trim($rdd_ffc_device_imei)."' and device_status not in(84,85,86,79,68,105,62,70,94,95,83,81,66,75,82,57,63,116,103,194,195,196,19)");

         }
        elseif($Device_type=="Deleted")
         {

                $query="update device_change set sales_manager='".$sales_manager."',device_model='".$device_model."', rdd_device_type='".$Device_type."', rdd_username='".$replace_client_id."', rdd_companyname='".$ReplaceCompany."', rdd_device_model='".$replace_Device_model."', rdd_device_imei='".$deleted_imei."', rdd_date='".$Txtrdd_date."', rdd_reason='".$TxtReason."', billing='".$billing2."', req_fill_date='".$date."' where id=$id";

                select_query($query);
                              
                $new_imei_query_inv = select_query_inventory("update inventory.device set device_status=65 where device_imei='".trim($deleted_imei)."' and device_status not in(84,85,86,79,68,105,62,70,94,95,83,81,66,75,82,57,63,116,103,194,195,196,19)");
      
                 $new_imei_query_inv2 = select_query_inventory("update inventory.device_repair set device_status=65 where current_record=1 and device_imei='".trim($deleted_imei)."' and device_status not in(84,85,86,79,68,105,62,70,94,95,83,81,66,75,82,57,63,116,103,194,195,196,19)");
               
         }
         else
         {

                 $query="update device_change set sales_manager='".$sales_manager."',device_model='".$device_model."', rdd_device_type='".$Device_type."', rdd_username='".$replace_user_id."', rdd_companyname='".$ReplaceCompany."', rdd_device_model='".$replaceDevice_model."', rdd_device_imei='".$replaceDeviceIMEI."',rdd_reg_no='".$veh_reg_replce_edit."', rdd_device_mobile_num='".$replaceDevicemobile."', rdd_date_replace='".$replacedate_of_install."', rdd_date='".$Txtrdd_date."', rdd_reason='".$TxtReason."', billing='".$billing1."', billing_reason='".$billing_reason."', req_fill_date='".$date."' where id=$id";

                select_query($query);
               
                $new_imei_query_inv = select_query_inventory("update inventory.device set device_status=65 where device_imei='".trim($replaceDeviceIMEI)."' and device_status not in(84,85,86,79,68,105,62,70,94,95,83,81,66,75,82,57,63,116,103,194,195,196,19)");
      
                $new_imei_query_inv2 = select_query_inventory("update inventory.device_repair set device_status=65 where current_record=1 and device_imei='".trim($replaceDeviceIMEI)."' and device_status not in(84,85,86,79,68,105,62,70,94,95,83,81,66,75,82,57,63,116,103,194,195,196,19)");
  ...
  else if($action=='editp')
  {
        if($Device_type=="New")
         {
                /* If Device Type New then Billing Default Yes.*/                           
               $query="update device_change set sales_manager='".$sales_manager."', device_model='".$device_model."', rdd_device_model='".$rdd_device_model."', rdd_date='".$Txtrdd_date."', rdd_reason='".$TxtReason."', billing='".$billing."' where id=$id";
         ...
         elseif($Device_type=="FFC")
         {
                ...
                $query="update device_change set sales_manager='".$sales_manager."', device_model='".$device_model."', rdd_device_model='".$rdd_ffc_device_model."', rdd_date='".$Txtrdd_date."', rdd_reason='".$TxtReason."',billing='".$billing_ffc."', billing_reason='".$billing_reason."' where id=$id";
        ...
        elseif($Device_type=="Deleted")
         {
                ...
                $query="update device_change set sales_manager='".$sales_manager."',device_model='".$device_model."', rdd_date='".$Txtrdd_date."', rdd_reason='".$TxtReason."', billing='".$billing2."' where id=$id";
         ...
         else
         {
                 ...
                 $query="update device_change set sales_manager='".$sales_manager."',device_model='".$device_model."', rdd_device_model='".$replaceDevice_model."', rdd_date='".$Txtrdd_date."', rdd_reason='".$TxtReason."', billing='".$billing1."', billing_reason='".$billing_reason."' where id=$id";
  ...
  else
  {
         ...
         if($Device_type=="New")
         {
          /* If Device Type New then Billing Default Yes.*/
                ...
                $query="INSERT INTO `device_change` (`date`,acc_manager, `sales_manager`, `client`, `user_id`, `device_model`, `device_imei`, `reg_no`, `date_of_install`, `mobile_no`, `rdd_device_model`, `rdd_device_imei`,`rdd_device_id`,`rdd_date`, `rdd_reason`,`rdd_device_type`,billing,req_fill_date) VALUES ('".$date."','".$acc_manager."','".$sales_manager."','".$client."','".$user_id."','".$device_model."','".$device_imei."','".$veh_reg."','".$date_of_install."','".$Devicemobile."','".$rdd_device_model."','".$rdd_device_imei."','".$get_device_id[0]['itgc_id']."','".$Txtrdd_date."','".$TxtReason."','".$Device_type."','".$billing."','".$billing_amount."','".$date."')";
                
                select_query($query);
                
                $new_imei_query = select_query_inventory("update inventory.device set device_status=65 where device_imei='".trim($rdd_device_imei)."' and device_status not in(84,85,86,79,68,105,62,70,94,95,83,81,66,75,82,57,63,116,103,194,195,196,19)");
      
                $new_imei_query2 = select_query_inventory("update inventory.device_repair set device_status=65 where current_record=1 and device_imei='".trim($rdd_device_imei)."' and device_status not in(84,85,86,79,68,105,62,70,94,95,83,81,66,75,82,57,63,116,103,194,195,196,19)");
                
         }
         else if($Device_type=="FFC")
         {

                $query="INSERT INTO `device_change` (`date`,acc_manager, `sales_manager`, `client`, `user_id`, `device_model`, `device_imei`, `reg_no`, `date_of_install`, `mobile_no`, `rdd_device_model`, `rdd_device_imei`, last_ffc_user, ffc_veh_no, ffc_date,`rdd_date`, `rdd_reason`,`rdd_device_type`,billing,billing_reason,req_fill_date) VALUES ('".$date."','".$acc_manager."','".$sales_manager."','".$client."','".$user_id."','".$device_model."','".$device_imei."','".$veh_reg."','".$date_of_install."','".$Devicemobile."','".$rdd_ffc_device_model."','".$rdd_ffc_device_imei."', '".$last_ffc_user."', '".$ffc_veh."','".$ffc_date."','".$Txtrdd_date."','".$TxtReason."','".$Device_type."','".$billing_ffc."','".$billing_amount."','".$billing_reason."','".$date."')";
                
                select_query($query);
                
                $new_imei_query = select_query_inventory("update inventory.device set device_status=65 where device_imei='".trim($rdd_ffc_device_imei)."' and device_status not in(84,85,86,79,68,105,62,70,94,95,83,81,66,75,82,57,63,116,103,194,195,196,19)");
      
                $new_imei_query2 = select_query_inventory("update inventory.device_repair set device_status=65 where current_record=1 and device_imei='".trim($rdd_ffc_device_imei)."' and device_status not in(84,85,86,79,68,105,62,70,94,95,83,81,66,75,82,57,63,116,103,194,195,196,19)");
        ...
        else if($Device_type=="Deleted")
         {
                ...
                $new_imei_query = select_query_inventory("update inventory.device set device_status=65 where device_imei='".trim($deleted_imei)."' and device_status not in(84,85,86,79,68,105,62,70,94,95,83,81,66,75,82,57,63,116,103,194,195,196,19)");
      
                   $new_imei_query2 = select_query_inventory("update inventory.device_repair set device_status=65 where current_record=1 and device_imei='".trim($deleted_imei)."' and device_status not in(84,85,86,79,68,105,62,70,94,95,83,81,66,75,82,57,63,116,103,194,195,196,19)");
         ...
         else
         {
                ...
                $new_imei_query = select_query_inventory("update inventory.device set device_status=65 where device_imei='".trim($replaceDeviceIMEI)."' and device_status not in(84,85,86,79,68,105,62,70,94,95,83,81,66,75,82,57,63,116,103,194,195,196,19)");
      
                $new_imei_query2 = select_query_inventory("update inventory.device_repair set device_status=65 where current_record=1 and device_imei='".trim($replaceDeviceIMEI)."' and device_status not in(84,85,86,79,68,105,62,70,94,95,83,81,66,75,82,57,63,116,103,194,195,196,19)");
   ...
   <table style=" padding-left: 100px;width: 500px;" cellspacing="5" cellpadding="5">
        ...
        <input type="text" name="date_of_install" id="date_of_install"  value="<?=$result[0]['date_of_install']?>" readonly/></td>
            ...
            <? if($action=='editp') {?>
                <tr>
                    <td>Replace with</td>
                    <td><input type="text" name="Device_type" id="Device_type" value="<?=$result[0]['rdd_device_type']?>" readonly="readonly" />
                    </td>
                </tr>
            ...
            <td colspan="2" align="center">
                 ...
                 <table  id="NewDevice" align="center" style="width: 500px;display:none;border:1"  cellspacing="5" cellpadding="5">
                    ...
                    <tr>
                        <td> <label id="lblDmodel">Device Id</label></td>
                        <td> <input type="text" name="device_id" id="TxtDModelNo" value="<?=$result[0]['rdd_device_id']?>" readonly="readonly" /></td>
                    </tr>
                    <tr>
                        <td> <label id="lblDmodel">Device IMEI</label></td>
                        <td> <input type="text" name="rdd_device_imei" id="rdd_device_imei" value="<?=$result[0]['rdd_device_imei']?>" readonly="readonly" /></td>
                    </tr>
                    ...
                    <tr>
                        <td> <label id="lblDmodel">Device IMEI</label></td>
                        <td><select name="rdd_device_imei" id="rdd_device_imei">
                         <option value="">-- Select IMEI --</option>
                        <?php
                            for($aid=0;$aid<count($assign_device);$aid++)
                            {
                            ?>
                             <option value="<?=$assign_device[$aid]['device_imei']?>" <? if($result[0]['rdd_device_imei']==$assign_device[$aid]['device_imei']) {?> selected="selected" <? } ?> >
                        <?php echo $assign_device[$aid]['device_imei']; ?> </option>
                            <?php
            ...
            <td colspan="2" align="center">
                ...
                <table  id="OldDevice" align="center"  style="width: 500px;display:none;border:1" cellspacing="5" cellpadding="5">
                    ...
                    <tr>
                        <td>
                        Client User Name</td>
                        <td><input type="text" name="replace_user_id" id="replace_user_id" value="<?=$main_user_iddata[0]['name']?>" readonly />
                        </td>
                    </tr>
            ...
            <td colspan="2" align="center">
                ...
                <table  id="deleteDevice" align="center"  style="width: 500px;display:none;border:1" cellspacing="5" cellpadding="5">
                    ...
                    <tr>
                        <td>
                        Client User Name</td>
                        <td><input type="text" name="replace_client_id" id="replace_client_id" value="<?=$main_user_iddata[0]['name']?>" readonly />
                        </td>
                    </tr>
