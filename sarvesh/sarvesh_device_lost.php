...
//echo "Update device_change set  pay_status=1  and device_old_paid_unpaid=1 where id ='".$chk_id[$i]."'";
 mysql_query("Update device_lost set  odd_paid_unpaid!=\"\" and ndd_paid_unpaid!=\"\"  where id ='".$chk_id[$i]."'");
...
$query = mysql_query("SELECT * FROM device_lost where odd_paid_unpaid=\"\" and ndd_paid_unpaid=\"\" order by id desc LIMIT $start,$end");   
?>
    <form method="POST">
...
<td><?php echo $row["acc_manager"];?></td>
  ...
  <td><a href="sarvesh_update_device_lost.php?id=<?php echo $row["id"];?>">Edit</a></td>
