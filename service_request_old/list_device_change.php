...
?>
        ...
        if($query[$i]["approve_status"]==0 && $query[$i]['req_fill_date']=='') {
    /*if(( $query[$i]["support_comment"]!="" or $query[$i]["admin_comment"]!="") && $query[$i]["final_status"]!=1 ) {*/?>
          ...
          <?php } if($query[$i]["approve_status"]==0 && $query[$i]['req_fill_date']!='') {?>
          <a href="devicechange.php?id=<?=$query[$i]['id'];?>&action=editp<? echo $pg;?>">Edit</a>
          <?php } }?></td>
        <td><?php if($_POST["Showrequest"]!=1 && $_POST["Showrequest"]!=2){    
